...
parser.add_argument("--mask-kind", type=str,  default="binary_apod",help='Mask version')
parser.add_argument("--mask-patch", type=str,  default=None,help='Mask patch')
parser.add_argument("--extract-mask", type=str,  default=None,help='Mask version')
...
parser.add_argument("-r", "--radial-fit-annulus",     type=int,  default=20,help="Number of sims.")
...
# Get data model
mask = sints.get_act_mr3_crosslinked_mask(mask_patch,version=args.mask_version,kind=args.mask_kind,season=args.season,array=args.array+"_f150")
...
for i in range(nsims):
    ...
    if args.extract_mask is not None: 
        sims = enmap.extract(sims,eshape,ewcs)
